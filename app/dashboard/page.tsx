...
export default async function DashboardPage() {
  ...
  if (user) {
    const [
      ...
      prisma.assignment.count({ where: { userId: user.id, status: 'pending' } }),
      prisma.note.findMany({ where: { userId: user.id }, orderBy: { createdAt: 'desc' }, take: 3 }),
      prisma.examPrediction.findMany({ where: { userId: user.id, examDate: { gte: new Date() } }, orderBy: { examDate: 'asc' }, take: 2 }),
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard title="Pending Tasks" value={pendingAssignments} icon={Target} description="Assignments due this week" />
        <StatCard title="Focus Time" value={focusTime} icon={Timer} description="Total focus time this week" />
        <StatCard title="Materials Studied" value={materialsCount} icon={BookOpen} description="Documents analyzed by AI" />
      </div>

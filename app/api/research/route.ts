...
export async function GET() {
  try {
    ...
    return apiError(error.message === 'Unauthorized' ? 'Unauthorized' : (error.message || 'Failed to fetch research projects'), error.message === 'Unauthorized' ? 401 : 500)
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    return apiError(error.message === 'Unauthorized' ? 'Unauthorized' : (error.message || 'Failed to process research request'), error.message === 'Unauthorized' ? 401 : 500)
  }
}

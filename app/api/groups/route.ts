...
export async function GET(req: Request) {
  try {
    ...
    return apiError(error.message === 'Unauthorized' ? 'Unauthorized' : (error.message || 'Failed to match study groups'), error.message === 'Unauthorized' ? 401 : 500)
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    return apiError(error.message === 'Unauthorized' ? 'Unauthorized' : (error.message || 'Failed to join study group'), error.message === 'Unauthorized' ? 401 : 500)
  }
}
